...
export default async function FuelPage() {
  ...
  return <div className="space-y-6">
    ...
    {locationOptions.length === 0
      ? <p className="rounded-xl border border-dashed border-stone-300 bg-white p-6 text-sm text-stone-600">Create an active fuel store before recording deliveries or issues.</p>
      : <>
    ...
    <Panel title="Recent movements">
      <div className="grid gap-6 lg:grid-cols-3">
        <div>
          <h3 className="text-sm font-semibold text-stone-500">Deliveries</h3>
          {receipts.data?.length
            ? <ul className="mt-2 space-y-2">{receipts.data.map((row) => {
                const location = Array.isArray(row.location) ? row.location[0] : row.location;
                return <li key={row.id} className="text-sm"><span className="font-medium">+{Number(row.litres).toLocaleString()} L</span> <span className="text-stone-600">{row.received_on}{location?.name ? ` · ${location.name}` : ""}{row.supplier ? ` · ${row.supplier}` : ""}</span></li>;
              })}</ul>
            : <p className="mt-2 text-sm text-stone-600">None recorded.</p>}
        </div>
        <div>
          <h3 className="text-sm font-semibold text-stone-500">Issues</h3>
          {issues.data?.length
            ? <ul className="mt-2 space-y-2">{issues.data.map((row) => {
                ...
                return <li key={row.id} className="text-sm"><span className="font-medium">−{Number(row.litres).toLocaleString()} L</span> <span className="text-stone-600">{row.issued_on}{equipmentRow?.name ? ` · ${equipmentRow.name}` : ""}{workerRow?.full_name ? ` · ${workerRow.full_name}` : ""}</span></li>;
              })}</ul>
            : <p className="mt-2 text-sm text-stone-600">None recorded.</p>}
        </div>
        <div>
          <h3 className="text-sm font-semibold text-stone-500">Adjustments</h3>
          {adjustments.data?.length
            ? <ul className="mt-2 space-y-2">{adjustments.data.map((row) => <li key={row.id} className="text-sm"><span className="font-medium">{Number(row.litres_delta) > 0 ? "+" : "−"}{Math.abs(Number(row.litres_delta)).toLocaleString()} L</span> <span className="text-stone-600">{row.adjusted_on} · {row.reason}</span></li>)}</ul>
            : <p className="mt-2 text-sm text-stone-600">None recorded.</p>}
        </div>
      </div>
    </Panel>
