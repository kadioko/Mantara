...
export default async function ProductionEntryPage({ params }: { params: Promise<{ entryId: string }> }) {
  ...
  return <div className="space-y-6">
    <div>
      <Link href="/production" className="text-sm font-semibold text-emerald-800 hover:underline">← Back to production</Link>
      <h1 className="mt-2 text-3xl font-bold">{entry.material}</h1>
      <p className="mt-1 text-stone-600">{productionStatusLabels[entry.status as keyof typeof productionStatusLabels] ?? entry.status} · {site.name}</p>
    </div>

    <section className="overflow-hidden rounded-xl border border-stone-200 bg-white">
      <div className="border-b border-stone-200 px-5 py-4"><h2 className="font-bold">Entry</h2></div>
      <div className="p-5">
        ...
        {entry.notes && <p className="mt-4 rounded-lg bg-stone-50 p-3 text-sm text-stone-700">{entry.notes}</p>}
      </div>
    </section>

    {(entry.status === "draft" || entry.status === "rejected") && canUpdate && <section className="rounded-xl border border-stone-200 bg-white p-5">
      <h2 className="font-bold">Submit</h2>
      <p className="mb-4 mt-1 text-sm text-stone-600">Send this entry for approval. Figures are frozen once approved.</p>
      <SubmitEntryForm entryId={entry.id} />
    </section>}

    {entry.status === "submitted" && canApprove && <section className="rounded-xl border border-stone-200 bg-white p-5">
      <h2 className="font-bold">Review</h2>
      <p className="mb-4 mt-1 text-sm text-stone-600">Approve or reject this submitted entry.</p>
      <ReviewForm entryId={entry.id} />
    </section>}

    {entry.status === "submitted" && !canApprove && <p className="rounded-xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-800">This entry is awaiting approval by someone with approval permission.</p>}

    <section className="overflow-hidden rounded-xl border border-stone-200 bg-white">
      <div className="border-b border-stone-200 px-5 py-4"><h2 className="font-bold">Approval history</h2></div>
      <div className="p-5">
        {approvals?.length
