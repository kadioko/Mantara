...
if(!secondUser){const {data,error}=await admin.auth.admin.createUser({email:process.env.QA_SECOND_EMAIL,password:process.env.QA_SECOND_PASSWORD,email_confirm:true,user_metadata:{full_name:"x10think RLS QA"}});if(error)throw error;secondUser=data.user;pass("Second restricted user","created isolated QA identity")}else pass("Second restricted user","reused isolated QA identity");
...
let secondScope=await scope(second);if(!secondScope){const {error}=await second.rpc("create_organization_with_owner",{organization_name:"x10think RLS QA",initial_site_name:"x10think Isolated Site",initial_site_country:"TZ"});if(error)throw error;secondScope=await scope(second);pass("Second tenant","created isolated organization and site")}else pass("Second tenant","reused isolated organization");
