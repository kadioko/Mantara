...
export function EquipmentForm() {
  ...
  return <form action={action} className="grid gap-4 rounded-xl border border-border bg-card p-5 md:grid-cols-2">
    ...
    <label className="text-sm font-semibold">Name *<input required name="name" maxLength={160} placeholder="CAT 320 excavator" className={fieldClass} /></label>
    ...
    <label className="text-sm font-semibold">Category *<select required name="category" defaultValue="excavator" className={selectClass}>{equipmentCategories.map((value) => <option key={value} value={value}>{categoryLabels[value]}</option>)}</select></label>
    ...
    <label className="text-sm font-semibold md:col-span-2">Notes<textarea name="notes" maxLength={2000} rows={3} className={fieldClass} /></label>
...
export function EquipmentStatusForm({ equipmentId, status }: { equipmentId: string; status: string }) {
  ...
  return <form action={action} className="grid gap-3 md:grid-cols-3">
    ...
    <label className="text-sm font-semibold md:col-span-2">Reason<input name="reason" maxLength={500} placeholder="Hydraulic leak reported by operator" className={fieldClass} /></label>
    <div className="md:col-span-3"><Feedback state={state} /></div>
