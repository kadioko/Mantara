...
export function EditEquipmentForm({ equipment }: { equipment: EquipmentDetails }) {
  ...
  return <form action={action} className="grid gap-4 md:grid-cols-2">
    ...
    <div><Label htmlFor="name">{tr("uiName")}</Label><Input id="name" name="name" required maxLength={160} defaultValue={equipment.name} className="mt-1" /></div>
    <div><Label htmlFor="assetCode">{tr("fAssetCode")}</Label><Input id="assetCode" name="assetCode" maxLength={80} defaultValue={equipment.asset_code ?? ""} className="mt-1" /></div>
    <div>
      <Label htmlFor="category">{tr("uiCategory")}</Label>
    ...
    <div>
      <Label htmlFor="meterType">{tr("uiMeterType")}</Label>
    ...
    <div><Label htmlFor="model">{tr("fModel")}</Label><Input id="model" name="model" maxLength={100} defaultValue={equipment.model ?? ""} className="mt-1" /></div>
    <div><Label htmlFor="serialNumber">{tr("fSerialNumber")}</Label><Input id="serialNumber" name="serialNumber" maxLength={120} defaultValue={equipment.serial_number ?? ""} className="mt-1" /></div>
    <div><Label htmlFor="yearOfManufacture">{tr("fYearOfManufacture")}</Label><Input id="yearOfManufacture" name="yearOfManufacture" type="number" min="1900" max="2100" step="1" defaultValue={equipment.year_of_manufacture ?? ""} className="mt-1" /></div>
    <div><Label htmlFor="acquiredOn">{tr("fAcquiredOn")}</Label><Input id="acquiredOn" name="acquiredOn" type="date" defaultValue={equipment.acquired_on ?? ""} className="mt-1" /></div>
    <div className="md:col-span-2"><Label htmlFor="notes">{tr("notes")}</Label><Textarea id="notes" name="notes" maxLength={2000} rows={2} defaultValue={equipment.notes ?? ""} className="mt-1" /></div>
...
export function RemoveEquipmentForm({ equipmentId, equipmentName }: { equipmentId: string; equipmentName: string }) {
  ...
  return <form action={action} className="space-y-3">
    ...
    <div>
      <Label htmlFor="confirmName">{tr("type")}<span className="font-semibold">{equipmentName}</span> to confirm</Label>
