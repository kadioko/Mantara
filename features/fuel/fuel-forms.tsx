...
export function FuelLocationForm() {
  ...
  return <form action={action} className="grid gap-4 rounded-xl border border-border bg-card p-5 md:grid-cols-3">
    ...
    <label className="text-sm font-semibold">Name *<input required name="name" maxLength={120} placeholder="Main diesel tank" className={fieldClass} /></label>
    <label className="text-sm font-semibold">Fuel type *<select required name="fuelType" defaultValue="diesel" className={selectClass}>{fuelTypes.map((value) => <option key={value} value={value}>{fuelTypeLabels[value]}</option>)}</select></label>
...
export function FuelReceiptForm({ locations, today }: { locations: Option[]; today: string }) {
  ...
  return <form action={action} className="grid gap-3 md:grid-cols-3">
    ...
    <label className="text-sm font-semibold">Litres *<input required name="litres" type="number" min="0.001" step="0.001" className={fieldClass} /></label>
    ...
    <label className="text-sm font-semibold">Supplier<input name="supplier" maxLength={160} className={fieldClass} /></label>
    <label className="text-sm font-semibold">Reference<input name="reference" maxLength={120} placeholder="Delivery note number" className={fieldClass} /></label>
    ...
    <label className="text-sm font-semibold md:col-span-3">Notes<input name="notes" maxLength={500} className={fieldClass} /></label>
    <div className="md:col-span-3"><Feedback state={state} /></div>
...
export function FuelAdjustmentForm({ locations, today }: { locations: Option[]; today: string }) {
  ...
  return <form action={action} className="grid gap-3 md:grid-cols-3">
    ...
    <label className="text-sm font-semibold md:col-span-2">Reason *<input required name="reason" maxLength={200} placeholder="Stock take variance" className={fieldClass} /></label>
