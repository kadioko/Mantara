...
export function DocumentUploadForm({ scope, ownerId }: { scope: DocumentScope; ownerId: string }) {
  ...
  async function upload(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!file) return setError(tr("chooseAFile"));
    if (file.size > MAX_FILE_BYTES) return setError(tr("fileTooLarge"));
    ...
    if (prepared.error || !prepared.path || !prepared.token) { setError(prepared.error ?? tr("uploadPreparationFailed")); setUploading(false); return; }
    const result = await createClient().storage.from(DOCUMENT_BUCKET).uploadToSignedUrl(prepared.path, prepared.token, file);
    if (result.error) { setError(tr("uploadFailed")); setUploading(false); return; }
    ...
    setMessage(recorded.success ?? tr("documentAttached")); form.reset(); setUploading(false); window.location.reload();
  }
  ...
  return <form ref={formRef} onSubmit={upload} className="grid gap-3 sm:grid-cols-2">
    ...
    <label className="text-sm font-semibold sm:col-span-2">{tr("uiFile")}<input required name="file" type="file" className={fieldClass} onChange={(event)=>{const selected=event.currentTarget.files?.[0];setError(selected&&selected.size>MAX_FILE_BYTES?tr("fileTooLarge"):null)}} /></label>
    ...
    <div><Button disabled={uploading||Boolean(error)}>{uploading?tr("uploading"):tr("attachDocument")}</Button></div>
  </form>;
}
