...
export function ItemRow({
  ...
  return (
    ...
    >
      <Field label="Name" required><input name="name" required maxLength={160} defaultValue={item.name} className={fieldClass} /></Field>
      <Field label="SKU"><input name="sku" maxLength={80} defaultValue={item.sku ?? ""} className={fieldClass} /></Field>
      <Field label="Unit" required><input name="unit" required maxLength={20} defaultValue={item.unit} className={fieldClass} /></Field>
      ...
      <Field label="Reorder level" hint="Leave blank for no reorder warning.">
...
export function StoreRow({ store, canManage }: { store: CatalogueStore; canManage: boolean }) {
  return (
    ...
    >
      <Field label="Name" required><input name="name" required maxLength={120} defaultValue={store.name} className={fieldClass} /></Field>
...
export function SupplierRow({ supplier, canManage }: { supplier: CatalogueSupplier; canManage: boolean }) {
  return (
    ...
    >
      <Field label="Name" required><input name="name" required maxLength={160} defaultValue={supplier.name} className={fieldClass} /></Field>
      <Field label="Contact"><input name="contactName" maxLength={160} defaultValue={supplier.contact_name ?? ""} className={fieldClass} /></Field>
      <Field label="Phone"><input name="phoneNumber" inputMode="tel" maxLength={40} defaultValue={supplier.phone_number ?? ""} className={fieldClass} /></Field>
      <Field label="Email"><input name="email" type="email" maxLength={200} defaultValue={supplier.email ?? ""} className={fieldClass} /></Field>
      <Field label="Notes" className="md:col-span-2">
